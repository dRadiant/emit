...
Measured single-threaded: **~420 blk/s** (recovery-bound) → parallelized per-block across a 10-worker slot pipeline (wider than the I/O-tuned 7, this pass is pure compute). **Full mainnet run, measured: 9,763,079 blocks in ~2.2 h (~1,250 blk/s sustained), txs.dat = 45.0 GB** (the bottom of the estimate band), 404 duplicate receipt groups resolved against the store, zero unrecovered senders, checksums verified throughout. Era-spanning parity: 887/887 log-producing txs across seven blocks (merge era through Dencun, Pectra, and near-tip) match RPC on every field. `to`/`value` extraction and the bodies point-get remain as designed.
